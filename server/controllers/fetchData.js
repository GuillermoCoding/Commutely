...
exports.IndeedJobs = async (args)=>{
        ...
        const indeedUrl = `http://api.indeed.com/ads/apisearch?publisher=${process.env.INDEED_API_KEY}&q=${title}&l=${zipcode}&start=${startingPage}&radius=1000&format=json&latlong=1&co=us&chnl=&userip=1.2.3.4&useragent=Mozilla/%2F4.0%28Firefox%29&v=2`;
        const response = await axios.get(indeedUrl);
        const filteredResponse = filterAsync(response.data.results, async ({jobtitle, company, city, state, country, url,latitude, longitude})=>{
